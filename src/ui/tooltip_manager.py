...
_NORMAL: dict[str, list[str]] = {
    ...
    "mode_combo": [
        "Advanced: choose how the alpha value is applied to each pixel in manual mode.",
        "'normalize' remaps the image's actual alpha range to [Min output, Max output] — no fixed value needed. This is the default.",
        "'set' replaces every pixel's alpha with the exact value you typed — useful for flat, uniform transparency.",
        "When using a preset, the preset's own mode is always used — this combo only applies in manual mode.",
    ],
    "alpha_spin": [
        ...
        "In 'set' mode, every processed pixel is set to exactly this value.",
        "Only active when 'Apply value to pixels' is checked in Advanced Options.",
        "Not used in 'normalize' mode — normalize remaps the range using Min/Max only.",
    ],
    "use_preset_check": [
...
_DUMBED: dict[str, list[str]] = {
    ...
    "mode_combo": [
        "Pick how the alpha gets applied. 'normalize' is the default — it remaps your alpha range to [Min, Max].",
        "Alpha mode. 'normalize' sets new alpha values. 'set' gives every pixel the same fixed value.",
        "Two options: normalize (default) or set. Normalize = remap. Set = everyone gets the same number.",
        "'normalize' = everyone gets a new alpha based on where they were in the range. Very fair.",
        "'set' = everyone gets the same alpha. 'normalize' = proportional remap. Both valid.",
    ],
    "alpha_spin": [
        ...
        "This number applies to every pixel in the image. Only used in 'set' mode.",
    ],
    "use_preset_check": [
...
_VULGAR: dict[str, list[str]] = {
    ...
    "alpha_slider": [
        ...
        "'set' mode carpet-bombs every pixel's alpha with your chosen value. Only used when 'Apply value' is checked.",
    ...
    "mode_combo": [
        "'normalize' remaps all alpha values proportionally to [Min, Max]. Use this first, genius.",
        "'normalize' = set new alphas in [Min, Max]. 'set' = everyone gets the same fixed number.",
        "Two modes: normalize (default) and set. Normalize = remap. Set = carpet-bomb with one value.",
        "'normalize' remaps the whole image's alpha range to [Min, Max]. No fixed value needed.",
        "'set' makes every pixel the same alpha value. 'normalize' keeps proportions but fits them in your range.",
        "normalize = proportional. set = bulldozer. Pick your weapon.",
        "Min and Max spinboxes control the output range. normalize uses both. set ignores them unless clamping.",
        "Start with normalize. Set Min and Max. Click Process. That's it.",
    ],
    "alpha_spin": [
        ...
        "Only used in 'set' mode. In normalize mode this does nothing — Min/Max control the output.",
        "Type your target alpha value here. Only matters if you're using 'set' mode.",
        ...
        "This number is the alpha that gets stamped on every pixel in 'set' mode. 0 = invisible. 255 = opaque wall.",
    ],
    "use_preset_check": [
